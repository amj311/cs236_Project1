...
TEST(FSA_ID, Lexer) {
	string test = "Q is an id.";
	Lexer lexer(test);
	lexer.tokenizeInput();
	vector<Token> tokens = lexer.getTokenList();
	string res = tokens[0].toString();
	EXPECT_EQ(res, "(ID,\"Q\",1)");
}
...
TEST(LexerTest, Undefined) {
	string test = "$Query";
	Lexer lexer(test);
	string res = lexer.findNextToken().toString();
	EXPECT_EQ(res, "(UNDEFINED,\"$\",1)");
}
TEST(LexerTest, UndefinedNum) {
	string test = "1Query";
	Lexer lexer(test);
	string res = lexer.findNextToken().toString();
	EXPECT_EQ(res, "(UNDEFINED,\"1\",1)");
}
